...
const parseNumber = (num: string) => ({
  float: Math.abs(Number.parseFloat(num)),
  int: Math.abs(Number.parseInt(num)),
  sign: Math.sign(Number(num)) < 0 ? '-' : ''
})
 ...
 */
export const numberEmptyState = (value: string, format: any = '---'): any => value === '0' || value === 'NaN' ? format : value
